...
::卸载组件
if exist idmBroker.exe start /wait idmBroker.exe -RegServer
if exist Uninstall.exe start /wait Uninstall.exe -uninstdriv
...
reg delete "HKLM\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\Internet Download Manager" /f >NUL 2>NUL
...
reg delete "HKLM\Software\WOW6432Node\Microsoft\Windows\CurrentVersion\Run" /v "IDMan" /f >NUL 2>NUL
...
reg delete "HKLM\SOFTWARE\WOW6432Node\Microsoft\Internet Explorer" /f /v "DownloadUI" >NUL 2>NUL
...
reg delete "HKCU\SOFTWARE\WOW6432Node\Microsoft\Internet Explorer" /f /v "DownloadUI" >NUL 2>NUL
